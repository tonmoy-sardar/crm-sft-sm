...
export class SourceComponent implements OnInit {
  ...
  getSources(){
    ...
    this.settingsService.getSources(param).subscribe(
      res=>{ 
        this.sourceList = res['results'];  
        this.totalSource = res['count'];
        this.itemNo = (this.defaultPagination - 1) * this.itemPerPage;
        this.lower_count = this.itemNo + 1;
        if (this.totalSource > this.itemPerPage * this.defaultPagination) {
          this.upper_count = this.itemPerPage * this.defaultPagination
        } else {
          this.upper_count = this.totalSource
        }     
  ...
  addSource(){   
    let dialogRef = this.dialog.open(AddComponent, {
      width: '525px',
      data: {title:'Add Source', from: 'add'}
    });
  ...
  editSource(data){   
    let dialogRef = this.dialog.open(AddComponent, {
      width: '525px',
      data: {title:'Edit Source', from: 'edit', details: data}
    });
  ...
  deleteSource(id:number,index:number){
    ...
    this.dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.loading = LoadingState.Processing;
        this.settingsService.deleteSource(id).subscribe(
          res=>{
